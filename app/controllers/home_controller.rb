...
class HomeController < ApplicationController
  ...
  def tool
    ...
    open(URI.encode('http://128.32.33.245:3003/mwords/get_data/?text='+params[:input_text])) { |f|
      ...
      @results = []
      for item in parsed_json
        @results << {:text => item['text'], :docs => item['docs'].collect do |x| all_docs[x] end, :length => item['length'], :score => item['score']}
      end
    }
  end
