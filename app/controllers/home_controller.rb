...
class HomeController < ApplicationController
  ...
  def tool
    ...
    puts '*************************abc****************************'
    begin
      open('http://128.32.33.245:3003/mwords/get_ngrams/?text='+URI.encode(params[:input_text])) { |f|
        response = ActiveSupport::JSON.decode(f.read)
        response_true = response['ngrams'].collect{|x| x if x['original']}.compact
        response_false = response['ngrams'].collect{|x| x if not x['original']}.compact
        response_true.concat(response_false)
        @results = {'processedtxt' => response['processedtxt'], 'ngrams' => response_true}
        puts '*************************cde*************************'
      }
    rescue
      @results = nil
